...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream()
                .filter(account -> Objects.equals(account.getBirthday().getMonth(), birthdayMonth))
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account -> Objects.equals(account.getSex(), Sex.MALE)));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account -> account.getCreationDate().getMonth(),
                        Collectors.mapping(Account::getBalance, Collectors.reducing(new BigDecimal("0"), BigDecimal::add))));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                .map(account -> account.getFirstName().chars().mapToObj(c -> (char) c))
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                ...
                .filter(string -> string.length() >= nameLengthBound)
