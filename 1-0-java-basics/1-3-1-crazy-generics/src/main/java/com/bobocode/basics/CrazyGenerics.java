...
public class CrazyGenerics {
     ...
     * @param <T> – a type of the entity that should be a subclass of {@link BaseEntity}
     */
    interface ListRepository<T extends BaseEntity> extends CrazyGenerics.CollectionRepository<T, List<T>> {
    }
    ...
    static class CollectionUtil {
        ...
        public static <T> Optional<T> findMax(Iterable<? extends T> elements, Comparator<? super T> comparator) {
            ...
            T maxValue = iterator.hasNext() ? iterator.next() : null;
            T temp;
            while (iterator.hasNext()) {
                temp = iterator.next();
                if (comparator.compare(temp, maxValue) > 0) {
                    maxValue = temp;
                }
            }
            return Objects.isNull(maxValue) ? Optional.empty() : Optional.of(maxValue);
        }
        ...
        public static <T extends BaseEntity> BaseEntity findMostRecentlyCreatedEntity(Collection<T> entities) {
            if (Objects.isNull(entities) || entities.size() < 1) {
